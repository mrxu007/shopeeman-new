...
class MallDataApi {
  ...
  async getShopMetricsvalue(item, api) {
    try {
      ...
      if (res.status === 200) {
        const resData = res && res.data && JSON.parse(res.data) && JSON.parse(res.data).data
        if (resData) {
          item['order_non_fulfillment_rate'] = (resData.non_fulfillment_rate * 100) + '%' // 订单未完成率（需上报）
        }
        return {
          code: 200,
          data: resData || {}
  ...
  async getKeyMetrics(item, api, params, type) {
    try {
      ...
      console.log(res)
      if (res.status === 200) {
        const resData = res && res.data && JSON.parse(res.data) && JSON.parse(res.data).result
        if (resData) {
          if (type === 'today') {
      ...
      if (res.status === 200) {
        ...
        if (resData) {
          if (type === 'real_time') {
            ...
            item.mall_datas.ChatResponseTimeData = formatDuring(resData.response_time.value).replace(/-/g, '') // 客服平均回应时间
  ...
  // 店铺数据（同步店铺数据）-账单验证接口，此步骤是输入密码验证 /api/v3/finance/get_bank_accounts/
  async getBankAccounts(item, api) {
    try {
      const {
        platform_mall_id,
        country
      } = item
      const mallData = await this._this.$appConfig.getGlobalCacheInfo('mallInfo', platform_mall_id)
      const mallJsonData = JSON.parse(mallData)
      const params = {
        'mallId': platform_mall_id,
        'password_hash': sha256(md5(mallJsonData.mall_account_info.password)),
        'page_size': 30,
        'page_number': 1
      }
      let res = await this._this.$shopeemanService.postChinese(country, api, params, {
        headers: {
  ...
  // 店铺数据（同步店铺数据）-获取待拨款订单数 /api/v3/finance/income_transaction_histories/
  async getFrozenAmountOrders(item, api) {
    try {
      const {
        platform_mall_id,
        country
      ...
      console.log(res)
      if (res.status === 200) {
        const resData = res && res.data && JSON.parse(res.data)
        if (resData) {
          item['frozen_amount_orders'] = resData.data.page_info.total // 待拨款订单数
        }
        return {
          code: 200,
          data: resData || {}
  ...
  async uploadMallData(item) {
    try {
      const params = {
        ...
        'displayResponseRate': item.display_response_rate,
        ...
        'lastweekAmount': item.lastweek_amount,
        'lastmonthAmount': item.lastmonth_amount,
        'availableAmount': item.available_amount,
        'frozenAmount': item.frozen_amount,
        'frozenAmountOrders': item.frozen_amount_orders,
        'mallDatas': {
