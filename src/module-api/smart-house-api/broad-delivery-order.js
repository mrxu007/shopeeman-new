...
export default class BroadDeliveryOrder {
  ...
  async getOutOfStockList(item) {
    try {
      ...
      ${this._this.$dayjs(item.created_time[0]).format('YYYY-MM-DD 00:00:00')}/
      ${this._this.$dayjs(item.created_time[1]).format('YYYY-MM-DD 23:59:59')}` : ''
      const pamars = JSON.parse(JSON.stringify(item))
