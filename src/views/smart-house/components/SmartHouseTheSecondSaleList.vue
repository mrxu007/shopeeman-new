    ...
    <el-row class="header">
      ...
      <ul>
        ...
        <li>
          <span>主订单号：</span>
          <el-input
            v-model="form.returnMainOrderNum"
            clearable
            size="mini"
            oninput="value=value.replace(/\s+/g,'')"
          />
        ...
        <li>
          <span>平台物流单号：</span>
          <el-input
            v-model="form.returnLogisticsDocNum"
            clearable
            size="mini"
            oninput="value=value.replace(/\s+/g,'')"
          />
      ...
      <ul><span style="color:red;margin-top:15px">温馨提示：此模块展示数据为海外仓退回仓库的包裹数据，若需将此模块包裹数据进行二次销售，请前往【订单管理】使用二次销售功能进行匹配（不支持拆包出库）</span></ul>
      <el-row id="article">
        <el-table
          ref="plTable"
          v-loading="Loading3"
