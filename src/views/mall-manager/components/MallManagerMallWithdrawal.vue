  ...
  export default {
    ...
    methods: {
      ...
      allTiedCard() {
        ...
        if (this.selectData.length || type) {
          if (this.selectIndex < 0) {
            this.selectIndex = 0
            this.active = this.selectData[this.selectIndex]
            let bank = this.bankList.filter(i => i.site === this.active.country)
            let bank_list = bank && bank[0]&& bank[0].bank_list || []
            let index = Math.floor(Math.random() * bank_list.length)
            this.bankCard = bank_list[index] || null
            this.tiedCardVisible = true
          } else {
            this.active = this.selectData[this.selectIndex] || null
            if (this.active) {
              let bank = this.bankList.filter(i => i.site.toLocaleUpperCase() === this.active.country)
              console.log(bank)
              let bank_list = bank && bank[0]&& bank[0].bank_list || []
              let index = Math.floor(Math.random() * bank_list.length)
              this.bankCard = bank_list[index] || null
      ...
      async getWalletOtpSeed(){
        let getWalletOtpSeedJson = await this.$shopeemanService.getWalletOtpSeed(this.active.country,{mallId: this.active.platform_mall_id})
