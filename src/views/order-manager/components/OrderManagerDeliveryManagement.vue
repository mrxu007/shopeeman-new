...
export default {
  ...
  methods: {
      ...
      try {
        ...
        for (let i = 0; i < filterArr.length; i++) {
          ...
          const padModel = {
            ...
            PlatformLogisticsName: order.logistics_name,
            ...
            SkuList: this.dealwithSkuList(order.goodsInfo),
    ...
    dealwithSkuList(skulist) {
      const resArr = []
      for (let i = 0; i < skulist.length; i++) {
        const info = `規格： ${skulist[i].variation_sku ? skulist[i].variation_sku.replace('=|=', '') : skulist[i].variation_name}  數量：${skulist[i].goods_count}`
        resArr.push(info)
      }
      return resArr
    },
      ...
      try {
        ...
        for (let i = 0; i < arrList.length; i++) {
          ...
          const params = {
            ...
            PlatformLogisticsName: orderInfo.logistics_name,
            ...
            SkuList: this.dealwithSkuList(orderInfo.goodsInfo),
        ...
        const pdfDownloadModel = {
          ...
          isGetSourceTW: this.isDownloadOriginFace711 || this.isDownloadOriginFaceQJ
        }
        // 5、-------------------downloadPdf
