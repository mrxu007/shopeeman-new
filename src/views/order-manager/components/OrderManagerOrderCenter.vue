<template>
  <div class="order-center">
        ...
        >
          <template slot-scope="{row,$index}">
            ...
            <p v-else-if="item.showType === 4"
              ...
              <el-link v-if="item.propLink && item.propLinkName" size="mini"
                  type="danger" @click="item.rowClick && tableRowBound(item.rowClick,row,$index,item) || ''">
                {{ item.propLinkName }}
              </el-link>
              <el-link
    ...
    >
      ...
      <goods-out-store :choose-data="uniqueArr(multipleSelection)" :out-store-type="outStoreType"
                       @dynamicSet="setTableData"
                       @close="closeDialog"/>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      tableColumnList: tableColumnListData,
  ...
  methods: {
    ...
    async uploadColumn() {
      ...
      const arr = []
      this.columnConfigShowList.forEach((item, index) => {
        arr.push({
    ...
    async getColumnsConfig() {
      try {
        ...
        console.log(data.data,'-------',this.columnConfigList)
        if (data.code === 200) {
    ...
    setColumnConfigShowList() {
      ...
      const list1 = []
      try {
        console.log(this.tableColumnList.map(i=>i.name))
        this.tableColumnList.forEach(item => {
