      ...
      if (this.mall.length > 0) {
        ...
        for (let i = 0; i < this.mall.length; i++) {
          ...
          if (attributeTreeRes.status === 200) {
            ...
            data['sales'] = `${this.currency}${attributeTreeRes.data.result.sales.value}`
            data['sales_per_buyer'] = `${this.currency}${attributeTreeRes.data.result.sales_per_buyer.value}`
