          ...
          <el-radio-group v-model="rewardType" @change="discountNum=''">
            ...
            <el-radio label="1">Shoppe币折扣
              <el-tooltip
                class="item"
                effect="dark"
                content="买家使用Shoppee币回扣优惠劵并获得Shoppee币作为回报。Shoppee币成本将由您吸收"
...
export default {
  ...
  methods: {
    ...
    async goodsCouponFun() {
      ...
      if (!this.useQuantity) {
        this.$message.warning('请输入有效优惠券数量')
        return
      }
    ...
    MallvoucherDelFun(val) {
      this.$confirm('确定要删除该优惠券吗？, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
      ...
      try {
        ...
        if (res.ecode === 0) {
          ...
          list.forEach(el => {
            ...
            el.voucher_type = el.rule.shopids?.length > 0 ? '店铺优惠券' : '商品优惠券'
    ...
    async createCoupon(val, count = { count: 1 }) {
      try {
        ...
        const params = {
          ...
          claim_quantity: '0',
          rule: {
            voucher_landing_page: discountShop ? '1' : '0',
        ...
        if (result.message === 'wrong voucher prefix') { // 若创建失败
          ...
          if (result.ecode === 0) {
            ...
            let message = ''
            if (result.message === 'fe param error') {
              message = '输入数据不正确，请重新输入'
            }
