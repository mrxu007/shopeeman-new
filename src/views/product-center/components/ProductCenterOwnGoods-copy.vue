  ...
  methods: {
    ...
    async exportData() {
      ...
      if (this.multipleSelection.length) {
        ...
        const returnCreateStartTime = this.form.returnCreateTime.length ? `${this.$dayjs(this.form.returnCreateTime[0]).format('YYYY-MM-DD HH:mm:ss')}` : ''
        const returnCreateEndTime = this.form.returnCreateTime.length ? `${this.$dayjs(this.form.returnCreateTime[1]).format('YYYY-MM-DD HH:mm:ss')}` : ''
        const returnUpdateStartTime = this.form.returnUpdateTime.length ? `${this.$dayjs(this.form.returnUpdateTime[0]).format('YYYY-MM-DD HH:mm:ss')}` : ''
        const returnUpdateEndTime = this.form.returnUpdateTime.length ? `${this.$dayjs(this.form.returnUpdateTime[1]).format('YYYY-MM-DD HH:mm:ss')}` : ''
        let params = {
          // 产品中心条件搜索
    ...
    async editGoods(row) {
      ...
      if (resObj.status_code === 200) {
        ...
        this.goodStatus = resObj.data.goods_type
        //处理sku
