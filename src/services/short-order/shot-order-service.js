...
export default class {
  ...
  async getShortOrders(buyerMap, payAccount, configInfo, nickInfo) {
    ...
    for (let index = 0; index < this.orders.length; index++) {
      ...
      const res = await this.$configService.getWarehouseInfo(itemOrder.mall_info.platform_mall_id)
      ...
      // 处理拍单姓名、地址、手机号
      const buyerInfoRes = await this.dealWithBuyerBaseInfo(warehouseInfo, itemOrder, configInfo, nickInfo)
      let buyerInfo = {}
  ...
  dealWithPurchaseList(itemOrder, nickInfo) {
    const shipList = []
    const res = this.goodBuyUrlList.find(item => {
      return item.goods_id === itemOrder.goods_info.goods_id
    })
    const execPlatform = /(yangkeduo.com)|(taobao.com)|(jingxi.com)|(jd.com)|(1688.com)|(tmall.com)|(pinduoduo.com)|(xiapi.xiapibuy.com)|(taobao.global)|(lazada.com)/g
    const execGoods = /goods_id=([0-9]*)/
    const execIDs = /id=([0-9]*)/
    const jxIDs = /sku=([0-9]*)/
    const tmGlobalIDs = /mpId=([0-8]*)/
  ...
  async dealWithBuyerBaseInfo(warehouseInfo, itemOrder, configInfo, nickInfo) {
    ...
    const namePhoneAddress = this.dealWithBuyName(warehouseInfo, itemOrder, configInfo, nickInfo)
    ...
    if (itemOrder.goods_info.ori_platform_id == 2 || itemOrder.goods_info.ori_platform_id == 3 || itemOrder.goods_info.ori_platform_id == 8) {
      ...
      const res = await window['BaseUtilBridgeService'].getLazadaAddress(warehouseInfo.distinct_id)
      ...
      if (warehouseInfo.country !== 'SG' && warehouseInfo.type === 3) {
        ...
        const resInfo = await window['BaseUtilBridgeService'].getLazadaAddress(warehouseInfo.distinct_id)
        console.log('处理shopee地址', resInfo)
        const allShopAdress = await this.getAllShopeeAddress(itemOrder.goods_info.ori_platform_id, warehouseInfo.country, warehouseInfo.shopee_map_id, 0)
        if (!allShopAdress) {
          return {
            code: 50001,
            data: '该仓库未映射shopee地址，请重新映射'
        ...
        const postCode = Number(resInfo.Post)
  ...
  async getAllShopeeAddress(platform, country, shopeeMapId, typeMark) {
    const address = {}
    while (true) {
      const shopeeAddrMappingTree = await this.searchShopeeAddrMappingTree(platform, country, shopeeMapId, typeMark)
      if (!shopeeAddrMappingTree || !shopeeAddrMappingTree.length) {
        break
        // return null
      }
      const addressLevel = shopeeAddrMappingTree[0].level
      const detailInfo = {}
