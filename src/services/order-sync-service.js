...
export default class {
    ...
    async tbsyncOrder(mall, orderItem) {
      ...
      for (let index = 0; index < subOrders.length; index++) {
        const GoodsID = await this.getGoodsID(mall, `https:${subOrders[index].itemInfo.itemUrl}`)
        console.log(subOrders[index])
        console.log(GoodsID)
        const newUrl = this.maxUrl(subOrders[index].itemInfo.pic)
        const obj = {
          goods_id: GoodsID,
    ...
    // 获取淘宝清晰大图链接
    maxUrl(url) {
      const jpeg = url.lastIndexOf('jpeg')
      console.log(jpeg)
      let newUrl = ''
      if (jpeg !== -1) {
    ...
    async allOrderDetai(mall) {
      ...
      for (let index = 0; index < orderList.length; index++) {
        ...
        const isflag = await this.syncOrderDetail(mall, orderList[index])
        this.writeLog(`正在同步【${orderList[index]}】订单详情信息`, isflag)
        this.delay(1000)
      }
    }
    async syncOrderDetail(mall, order) {
      const url = 'https://trade.taobao.com/trade/detail/trade_order_detail.htm'
      ...
      if (str) {
        ...
        const cloendata = _.cloneDeep(data)
    ...
    async getOrderDetail(mall, order) {
      try {
        ...
        const orderDetailSerices = new orderDetail(response.data)
