...
export default class {
  ...
  async tbsyncOrder(mall, orderItem) {
    ...
    for (let index = 0; index < subOrders.length; index++) {
      const { goodsID, skuPic } = await this.getGoodsID(
        mall,
  ...
  async getGoodsID(mall, url) {
    ...
    let skuPic
    const goodsID = str.match(/\\"itemId\\":([0-9]*)/)
  ...
  getRefundList(mall) {
    ...
    this.jdNetworkService
      ...
      .then(response => {
        let page = 1
        const regx = response.data.match(/var disputeData.*/)
        if (regx) {
          const disputeData = JSON.parse(
          ...
          this.procDisputeData(disputeData, page, mall).then(async() => {
            ...
            const pageSize = parseInt(this.disputePagination.defaultPageSize)
            const total = parseInt(this.disputePagination.total)
      ...
      this.jdNetworkService
        ...
        .then(response => {
          if (response.data.success) {
            const disputeData = response.data.resultData
            // console.log(disputeData)
            resolve(disputeData)
          } else {
  ...
  async procDisputeData(disputeData, page, mall) {
    ...
    const gridItems = disputeData.linkage.input
    console.log(newArry, 'newArry')
    let listIndex = -1
    console.log(this.disputes, '测试')
    newArry.forEach((name, index) => {
      // if (name.indexOf('ListGrid') >= -1) {
      const dispute = disputeData.data[name]
      const id = dispute.id.split('@')[0]
      const exist = this.disputes.filter(item => {
      ...
      if (dispute.bizName === 'disputeBody') {
        ...
        if (newdisputes[listIndex]) {
          newArry.forEach(item => {
            const disputes = disputeData.data[item]
            if (
              item ===
  ...
  async tbsyncOrder(mall, orderItem) {
    ...
    const status = this.orderafterStatus(order_after_status)
    const order = {
      sys_mall_id,
      order_id,
      after_id,
  ...
  afterReason(str) {
    ...
    const matchvalue = array.get(str)
    if (!matchvalue) {
      return 0
    }
    return matchvalue
